...
namespace ChatAPI.Controllers
{
    ...
    public class ChatController : ControllerBase
    {
        ...
        [HttpGet("conversation/{conversationId}/members")]
        public async Task<IActionResult> GetGroupMembers(string conversationId)
        {
            var userId = User.FindFirstValue(ClaimTypes.NameIdentifier);
            if (userId == null) return Unauthorized();

            var conv = await _convRepo.GetByIdAsync(conversationId);
            if (conv == null || !conv.Members.Contains(userId)) return NotFound();

            var members = await _userRepo.GetByIdsAsync(conv.Members);
            var views = members.Select(u => new UserViewDto
            {
                Id = u.Id,
                Username = u.Username,
                DisplayName = u.DisplayName,
        ...
        [HttpPost("conversation/{conversationId}/members")]
        public async Task<IActionResult> AddGroupMembers(string conversationId, [FromBody] AddGroupMembersDto dto)
        {
            var userId = User.FindFirstValue(ClaimTypes.NameIdentifier);
            if (userId == null) return Unauthorized();

            var conv = await _convRepo.GetByIdAsync(conversationId);
            if (conv == null || conv.Type != "group") return NotFound();

            if (conv.AdminId != userId)
                return BadRequest(new { message = "Only the group admin can add members." });

            var newMembers = dto.UserIds.Where(id => !conv.Members.Contains(id)).Distinct().ToList();
            if (newMembers.Any())
            {
                conv.Members.AddRange(newMembers);
